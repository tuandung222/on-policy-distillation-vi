...
- **Off-policy distillation** huấn luyện student trên dữ liệu cố định — thường là các logits hoặc câu trả lời đã được tính toán trước từ teacher.
- **On-policy distillation** cho phép teacher phản hồi trực tiếp trên chính những đầu ra mà student tự tạo ra.
...
**GKD** (Agarwal et al., 2024) thống nhất cả hai phương pháp trên dưới một khung lý thuyết chung. GKD hỗ trợ nhiều hàm mất mát khác nhau và cho phép huấn luyện trên cả dữ liệu tĩnh của teacher lẫn các chuỗi (trajectories) do student tự tạo ra. Bài báo GKD chỉ ra rằng **on-policy distillation thường vượt trội hơn các phương pháp off-policy**.

### Tại sao On-policy hiệu quả hơn?

Ưu thế của on-policy distillation đến từ hai yếu tố:

1. **Vòng phản hồi tích cực:** Khi student cải thiện, các câu trả lời nó tạo ra ngày càng chất lượng hơn, tạo thành dữ liệu huấn luyện tốt hơn theo thời gian.
2. **Căn chỉnh ngữ cảnh:** Phương pháp này buộc student phải học từ chính những loại lỗi và thành công mà nó sẽ gặp trong quá trình suy luận thực tế.
...
- $p_S(.|x)$ là phân phối xác suất của student khi nhận đầu vào $x$
- $\mathcal{D}_{JSD(\beta)}$ là Jensen-Shannon Divergence với tham số $\beta$ điều chỉnh trọng số

### So sánh với Reinforcement Learning

So với RL, GKD có hai lợi thế chính:
...
Điểm yếu của **tất cả** các phương pháp on-policy distillation trước đây là giả định teacher và student dùng **cùng một tokenizer**. Mỗi họ mô hình có tokenizer riêng, nên ràng buộc này rất khắt khe khi chọn cặp student-teacher.

### Hai thách thức khi tokenizer khác nhau

**ULD** (Boizard et al., 2025) chỉ ra rằng chưng cất giữa các mô hình có tokenizer khác nhau dẫn đến hai thách thức chính:
...
Cùng một chuỗi ký tự nhưng nhận **ID khác nhau** trong từ vựng của mỗi tokenizer.
...
Đây chính là hai vấn đề mà **GOLD** được thiết kế để giải quyết, như chúng ta sẽ thấy trong chương tiếp theo.
